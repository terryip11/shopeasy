...
export function MerchantDeliveryCell({
  ...
  const [pickup, setPickup] = useState(defaultPickupAddress || '');
  ...
  useEffect(() => {
    if (open) {
      ...
      setPickup(defaultPickupAddress || '');
    }
  }, [open, shippingAddress, shippingZoneId, defaultJobType, defaultPickupAddress]);
      ...
      {open && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/50 p-4">
          <div className="w-full max-w-md rounded-2xl bg-white p-6 shadow-xl dark:bg-gray-900">
            ...
            {shippingAddress && (
              <p className="mt-2 text-xs text-green-600">已帶入買家收貨地址</p>
            )}
            ...
            <div className="mt-4 space-y-3">
              ...
              <div>
                ...
                {defaultPickupAddress && pickup === defaultPickupAddress && (
                  <p className="mt-1 text-xs text-green-600">已帶入店鋪營業地址</p>
                )}
              </div>
              <div>
                <Label>送達地址 *</Label>
                <Input value={dropoff} onChange={(e) => setDropoff(e.target.value)} className="mt-1" required />
              </div>
            </div>
