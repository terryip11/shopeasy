...
import { StoreShareButton } from '@/components/marketing/store/store-share-button';
...
export function StoreHeader({ storeSlug, storeName, initialQuery = '' }: Props) {
  ...
  const handleSearch = (e: React.FormEvent) => {
    ...
    router.push(qs ? `${storeBase}?${qs}` : storeBase);
  };
  ...
  return (
    <header className="sticky top-0 z-50 border-b border-gray-100 bg-white/95 backdrop-blur-md dark:border-gray-800 dark:bg-gray-950/95">
      {/* 桌面：麵包屑 */}
      <div className="mx-auto hidden max-w-7xl items-center gap-1.5 px-6 py-2 text-sm text-gray-500 md:flex">
        <Link href="/products" className="hover:text-orange-600">
          ShopEasy
        </Link>
        <ChevronRight className="h-4 w-4 shrink-0" />
        <span className="truncate font-medium text-gray-900 dark:text-white">{storeName}</span>
      </div>

      <div className="mx-auto flex max-w-lg items-center gap-2 px-3 py-2.5 md:max-w-7xl md:gap-3 md:px-6 md:py-3">
        <Link
          href="/products"
        ...
        <StoreShareButton storeName={storeName} />
        <Link
          href="/cart"
