...
type Props = {
  /** Server 預取：略過 client /api/me → addresses 串行 */
  initialLoggedIn?: boolean;
  initialAddresses?: BuyerAddress[];
};
...
export function CheckoutForm({
  ...
  const [savedAddresses] = useState<BuyerAddress[]>(initialAddresses);
  ...
  useEffect(() => {
    ...
    // Server 已注入地址：無 draft 時套用預設地址（不再打 /api/me、/api/buyer/addresses）
    if (initialLoggedIn && initialAddresses.length > 0 && !restoredFromDraft.current) {
      const defaultAddr =
    ...
    // 僅 mount 一次：地址由 Server 注入，options 依 localStorage cart
