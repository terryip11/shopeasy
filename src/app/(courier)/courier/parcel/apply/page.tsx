...
export default async function ParcelCourierApplyPage() {
  const [zones, profile] = await Promise.all([getDeliveryZones(), getCourierProfile()]);

  if (profile?.status === 'active') {
    return (
      <CourierMobileShell variant="parcel" activeTab="hub" title="已是配送員" showBack>
        <div className="rounded-2xl border border-gray-200 bg-white p-6 text-center dark:border-gray-800 dark:bg-gray-900">
          ...
          <Button asChild className="mt-4 w-full">
            <Link href="/courier/parcel">前往送貨工作台</Link>
          </Button>
        </div>
      </CourierMobileShell>
    );
  ...
  if (profile?.status === 'pending') {
    return (
      <CourierMobileShell variant="parcel" activeTab="hub" title="審核中" showBack>
        <div className="rounded-2xl border border-gray-200 bg-white p-6 text-center dark:border-gray-800 dark:bg-gray-900">
          <h2 className="text-lg font-bold">申請審核中</h2>
          <p className="mt-2 text-sm text-gray-500">請耐心等待平台審核。</p>
          <Button asChild variant="outline" className="mt-4 w-full">
            <Link href="/courier/parcel">查看狀態</Link>
          </Button>
        </div>
      </CourierMobileShell>
    );
  ...
  return <CourierApplyForm zones={zones} jobType="parcel" />;
}
